# Asset Director — Trivia Reaction Pipeline
...
```
Model:        Seedance 2.0
Character:    ellie.travelcrush      (saved character in your library)
Aspect:       9:16
Resolution:   1080p
Duration:     15s
...
mp4 to `scripts/trivia_reaction/library/clips/<slug>.mp4`, then say go.
...
Once a clip exists at `scripts/trivia_reaction/library/clips/<slug>.mp4`,
quick sanity:
...
`projects/trivia-reaction/<slug>/artifacts/asset_manifest.json`:
  ...
  "assets": [
    {
      "type": "video",
      "role": "avatar_clip",
      "path": "scripts/trivia_reaction/library/clips/<slug>.mp4",
      "duration_s": 15.2,
      "aspect": "1080x1920",
      "fps": 30,
      "generation_summary": {
...
- Do not silently swap the character to a different saved character or
  to a text-only describe-her prompt. ellie.travelcrush is the lock.
