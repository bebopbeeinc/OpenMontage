# Executive Producer — Trivia Reaction Pipeline

## When To Use

User references the trivia-reaction workflow. Triggers:
...
"I just found out" reaction-reel format for the ellie.travelcrush account.

## Philosophy

Trivia-reaction is **sheet-authoritative on two surfaces**:
...
2. **TriviaReactionQueue!Queue** owns the *workflow state* — Slug, Status,
   the three VO lines, Drive Link, and the assembled OpenArt Prompt.
   Pipeline writes here at select / script / compose / publish boundaries.

   Status is a 4-state enum: `Draft` → `Ready to review` → `Ready to publish`
   → `Published`. See `queue_row.STATUS_*` constants.

Artifacts under `projects/trivia-reaction/<slug>/artifacts/` are derived views — they
...
2. **Run the user's house defaults.** No music; orange-pill captions;
...
3. **OpenArt character lock.** Every video gen call uses
   `character="ellie.travelcrush"`. Never describe ellie's face from text
   in the per-row prompt — the OpenArt saved character carries her identity.
...
| TriviaReactionQueue | Queue | all stages | select, script, compose, publish |

Within TriviaReactionQueue!Queue:
...
| Avatar clip change ("ellie's wrong"/"backdrop wrong") | `script` → `assets` → `edit` → `compose` | OpenArt prompt edit + regen |
...
- `pipelines/trivia-reaction/select-director` — Day → brief
- `pipelines/trivia-reaction/script-director` — brief → three beats + OpenArt prompt
- `pipelines/trivia-reaction/asset-director` — manual vs automated OpenArt
- `pipelines/trivia-reaction/edit-director` — assemble.py orchestration
- `pipelines/trivia-reaction/compose-director` — transcribe + Remotion + frame review
- `pipelines/trivia-reaction/publish-director` — Drive + Queue write-back
- `meta/reviewer`
...
- Do not describe ellie's face in OpenArt prompts. Use the character ref.
