# Compose Director — Trivia Reaction Pipeline
...
2. Running Remotion's `TriviaWithBg` composition with
   `showFactsOverlay=false` + `highlightColor=#FF6A2C` against
...
|---|---|---|
| Artifact | `projects/trivia-reaction/<slug>/artifacts/edit_decisions.json` | bg + vo paths, render runtime |
| Asset | `projects/trivia-reaction/<slug>/assets/video/bg.mp4` | Background |
| Asset | `projects/trivia-reaction/<slug>/assets/audio/vo.wav` | Narration |
| Asset | `projects/trivia-reaction/<slug>/assets/meta.json` | Remotion props bundle |
...
`tools/analysis/transcriber.py`. Whisper's `base.en` model is usually
enough; bump to `small.en` if word boundaries drift > 200ms.

### 2. Render Remotion

The composition is `TriviaWithBg` (reused from trivia-short with
`showFactsOverlay=false`). Trivia-reaction's locked props:

| Prop | Value |
|---|---|
| `highlightColor` | `#FF6A2C` |
| `baseColor` | `#FFFFFF` |
| `fontSize` | `78` |
...
npx remotion render src/index-trivia.tsx TriviaWithBg \
  ../projects/trivia-reaction/<slug>/renders/<slug>.mp4 \
  --props=../projects/trivia-reaction/<slug>/assets/remotion_props.json
```
...
"ready". For trivia-reaction the beats are at known offsets in
`meta.json.beat_offsets_ms`. Extract one frame per beat midpoint:

```bash
for beat in hook fact kicker; do
  midpoint_s=$(python -c "import json; m=json.load(open('projects/trivia-reaction/<slug>/assets/meta.json')); o=m['beat_offsets_ms']['$beat']; print(o/1000.0 + 0.5)")
  ffmpeg -y -ss $midpoint_s -i projects/trivia-reaction/<slug>/renders/<slug>.mp4 \
    -frames:v 1 projects/trivia-reaction/<slug>/renders/_review_$beat.png
done
```
...
- Orange pill (`#FF6A2C`) is rendered on the active word
- Caption is bottom-aligned, no clipping
- bg.mp4 frame shows ellie in the expected acting beat
...
| Caption drift > 200ms | Re-transcribe with `small.en` model; re-render |
| Orange pill not showing | Verify `remotion_props.json` has the right highlightColor; rebuild props; re-render |
...
```jsonc
{
  ...
  "output_path": "projects/trivia-reaction/<slug>/renders/<slug>.mp4",
  "metadata": {
    "sheet_revision": "...",
    "slug": "<slug>",
    ...
    "review_frames": {
      "hook": "projects/trivia-reaction/<slug>/renders/_review_hook.png",
      "fact": "projects/trivia-reaction/<slug>/renders/_review_fact.png",
      "kicker": "projects/trivia-reaction/<slug>/renders/_review_kicker.png"
    },
    "defects": [],
    "fixes_applied": []
  }
