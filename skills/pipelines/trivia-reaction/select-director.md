# Select Director — Trivia Reaction Pipeline
...
4. Upsert the TriviaReactionQueue row with status `Draft`.
...
| Sheet | TriviaReactionQueue!Queue | Workflow-state SoT |
| Script | `scripts/trivia_reaction/select_row.py` | The CLI that does all of this |
...
```bash
python scripts/trivia_reaction/select_row.py --day <N>
```
...
```bash
python scripts/trivia_reaction/select_row.py --day <N> --slug <custom-slug>
```

To inspect without writing:

```bash
python scripts/trivia_reaction/select_row.py --day <N> --dry-run
```

### 3. Read Back The Brief

The CLI writes `projects/trivia-reaction/<slug>/artifacts/brief.json`. Read it before
checkpointing — the trivia_reaction sub-object inside `metadata` is what
