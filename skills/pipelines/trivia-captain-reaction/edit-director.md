# Edit Director — Trivia Captain Reaction Pipeline

## When To Use

You are running `scripts/trivia_captain_reaction/assemble.py` to produce two
inputs to the Remotion compose stage:

1. `projects/trivia-captain-reaction/<slug>/assets/video/bg.mp4` — the avatar clip normalized to
...
2. `projects/trivia-captain-reaction/<slug>/assets/meta.json` — Remotion-facing config:
...
produce word-level timestamps for the warm purple-pill captions.

## Prerequisites

| Layer | Resource | Purpose |
|---|---|---|
| Artifact | `projects/trivia-captain-reaction/<slug>/artifacts/brief.json` | Slug, Day, sheet revision |
| Artifact | `projects/trivia-captain-reaction/<slug>/artifacts/script.json` | Beat copy (for compose to align captions) |
| Artifact | `projects/trivia-captain-reaction/<slug>/artifacts/asset_manifest.json` | Avatar clip path |
| Library | `scripts/trivia_captain_reaction/library/clips/<slug>.mp4` | Seedance clip with native audio |
| Script | `scripts/trivia_captain_reaction/assemble.py` | Orchestrator (this stage's tool) |
...
ffprobe -v error -select_streams a:0 -show_entries stream=codec_type \
  ...
  scripts/trivia_captain_reaction/library/clips/<slug>.mp4
...
- `meta.json` exists; `remotion_props.highlightColor == "#C04FE0"`,
