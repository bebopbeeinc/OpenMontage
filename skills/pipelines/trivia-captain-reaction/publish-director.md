# Publish Director — Trivia Captain Reaction Pipeline
...
|---|---|---|
| Artifact | `projects/trivia-captain-reaction/<slug>/artifacts/render_report.json` | Render path + frame_review_passed=true |
...
```bash
python scripts/trivia_captain_reaction/publish.py <slug>
```

What it does:

1. Locate both deliverables:
   - `projects/trivia-captain-reaction/<slug>/renders/<slug>.mp4` — captioned
     final render (the posted version)
   - `scripts/trivia_captain_reaction/library/clips/<slug>.mp4` — raw Seedance
...
```jsonc
{
  ...
  "sheet_updates": [
    { "range": "Posts_Reaction!C<row>", "value": "Ready to publish" },
    { "range": "Posts_Reaction!I<row>", "value": "https://drive.google.com/..." }
  ],
...
`publish.py` writes the Drive link to Posts_Reaction!I (col I = Drive Link;
col J is the OpenArt Prompt). The human picks it up
