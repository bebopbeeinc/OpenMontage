# Select Director — Trivia Captain Pipeline
...
4. Upsert the TriviaCaptainQueue row with status `Draft`.
...
which Days make good captain-reel candidates; the heuristic was
redundant. If a Day doesn't fit (vocab quiz, flat answer), just don't
queue it.

**Persona-fit note.** Prefer facts where Archibald can claim plausible
proximity — a place, region, or experience he could have "been there
in '87" for. The comedy engine is that he was RIGHT THERE and nobody
told him. Facts with no travel/place/experience angle (pure science
trivia, abstract definitions) are weak fits: flag them when you surface
the row so the human can decide whether to queue or skip.
...
| Sheet | TriviaCaptainQueue!Queue | Workflow-state SoT |
| Script | `scripts/trivia_captain/select_row.py` | The CLI that does all of this |
...
```bash
python scripts/trivia_captain/select_row.py --day <N>
```
...
```bash
python scripts/trivia_captain/select_row.py --day <N> --slug <custom-slug>
```

To inspect without writing:

```bash
python scripts/trivia_captain/select_row.py --day <N> --dry-run
```

### 3. Read Back The Brief

The CLI writes `projects/trivia-captain/<slug>/artifacts/brief.json`. Read it before
checkpointing — the trivia_captain sub-object inside `metadata` is what
...
- The persona-fit read (strong proximity angle vs weak/no-place fit)
