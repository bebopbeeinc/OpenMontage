# Executive Producer — Trivia Captain Pipeline

## When To Use

User references the trivia-captain workflow. Triggers:

- "render Day <N>" / "make a captain reel for Day <N>"
...
For a one-off short, the daily trivia mobile game, or any non-captain
format, pick a different pipeline. This pipeline is specific to the
cold-open incredulity reel format for the @captain.archibald account.

## Philosophy

Trivia-captain is **sheet-authoritative on two surfaces**:
...
2. **TriviaCaptainQueue!Queue** owns the *workflow state* — Slug, Status,
   the three VO lines, Drive Link, the assembled OpenArt Prompt, and the
   reference-image path. Pipeline writes here at select / script / compose
   / publish boundaries.

   Status is a 4-state enum: `Draft` → `Ready to review` → `Ready to publish`
   → `Published`. See `queue_row.STATUS_*` constants.

Artifacts under `projects/trivia-captain/<slug>/artifacts/` are derived views — they
...
2. **Run the user's house defaults.** No music; warm-red-pill captions;
...
3. **OpenArt character lock.** Every video gen call uses
   `character="Captain Archibald"`. Never describe Archibald's face from
   text in the per-row prompt — the OpenArt saved character carries his
   identity.
...
| TriviaCaptainQueue | Queue | all stages | select, script, compose, publish |

Within TriviaCaptainQueue!Queue:
...
| M | Reference Image | asset-director (splash uploaded to OpenArt as tablet ref) | script-director |
...
| Avatar clip change ("Archibald's wrong"/"backdrop wrong"/"splash wrong") | `script` → `assets` → `edit` → `compose` | OpenArt prompt or reference-image edit + regen |
...
- **Asset stage (automated_openart):** announce model + variant count +
  character lock + reference image + duration before driving OpenArt.
...
- `pipelines/trivia-captain/select-director` — Day → brief
- `pipelines/trivia-captain/script-director` — brief → cold-open hook + fact + kicker + OpenArt prompt
- `pipelines/trivia-captain/asset-director` — manual vs automated OpenArt
- `pipelines/trivia-captain/edit-director` — assemble.py orchestration
- `pipelines/trivia-captain/compose-director` — transcribe + Remotion + frame review
- `pipelines/trivia-captain/publish-director` — Drive + Queue write-back
- `meta/reviewer`
...
- Do not describe Archibald's face in OpenArt prompts. Use the character ref.
