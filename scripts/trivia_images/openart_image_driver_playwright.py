...
# to `data-icon`: on the new UI `document.querySelectorAll('svg[aria-label]')`
# returns nothing at all, which silently broke every selector that reached for
# an icon by name (the workspace switcher, the Model and Setting cards, the
# reference-chip remove button). The icon *names* are unchanged, so we match
# either attribute rather than pinning to the new one — the header still offers
# a "Previous version" toggle, and an account flipped back must keep working.
...
class Selectors:
    ...
    # The refresh also dropped the card's "Setting" text label — it now reads
    # "Output" over a "4:3 | 1K" summary — so the new form is identified by the
    # Setting icon instead. Both spellings are matched.
    setting_card: str = (
    ...
    # Removing a reference chip: each chip's root is the nearest ancestor <div>
    # carrying Tailwind's `group` class; the remove control is a sibling button
    # pinned to its top-right corner, identified by its CloseBold icon. The
    # button is always in the DOM (it animates in via group-hover:opacity), so
    # it can be clicked without a real hover.
    reference_chip_root: str = "xpath=ancestor::div[contains(@class,'group')][1]"
    reference_chip_remove: str = f"button:has({_icon('CloseBold')})"
...
# How long to keep trying to remove leftover reference chips before giving up.
REFERENCE_CLEAR_TIMEOUT_S = 20


def _attach_reference_image(page: Page, image_path: Path) -> None:
    """Upload `image_path` as the *only* reference image and wait for the
    server upload to settle.
    ...
    Two things this deliberately checks rather than assumes:

      - The form must be empty first. Any pre-existing chip is a leftover from
        an earlier row (see FORM_STATE_KEY) and would be sent to the model
        alongside ours, so we clear it and fail loudly if it won't go.
      - The settle gate counts *settled* previews against the *total*, not
        "is any CDN preview present". The old global `count() > 0` check was
        satisfied by a leftover chip that was already on the CDN, so it
        returned while our own upload was still a blob: URL — exactly the
        submit-too-early case the wait exists to prevent.
    """
    if not image_path.exists():
        raise FileNotFoundError(f"reference image not found: {image_path}")

    _clear_reference_images(page)
    ...
    # Then wait for every preview to carry a CDN URL, indicating server-side
    # ingestion. With the form cleared above, "every" means our one upload.
    deadline = time.time() + REFERENCE_UPLOAD_TIMEOUT_S
    total = settled = 0
    while time.time() < deadline:
        total = page.locator(SEL.reference_preview).count()
        settled = page.locator(SEL.reference_cdn_preview).count()
        if total == 1 and settled == 1:
            print(
            ...
            return
        if total > 1:
            _diagnose(page, "reference_stacked")
            raise RuntimeError(
                f"{total} reference images attached, expected 1 — the form "
                f"was not clean (image={image_path})",
            )
        time.sleep(1.0)
    raise RuntimeError(
        f"reference upload never settled to a CDN URL within "
        f"{REFERENCE_UPLOAD_TIMEOUT_S}s ({settled}/{total} settled, "
        f"image={image_path})",
    )


def _clear_reference_images(page: Page) -> None:
    """Remove any reference chips already sitting in the Create-Image form.

    Belt-and-braces alongside stripping FORM_STATE_KEY from the storage state:
    that keeps references from leaking across *runs*, this keeps them from
    stacking *within* one run — the out-of-credits fallback refills the form a
    second time, and the reload in between does not reset the live store.

    Best-effort per chip; the count check at the end is the real gate.
    """
    chips = page.locator(SEL.reference_preview)
    if chips.count() == 0:
        return

    print(f"  → clearing {chips.count()} stale reference chip(s)", file=sys.stderr)
    deadline = time.time() + REFERENCE_CLEAR_TIMEOUT_S
    while chips.count() > 0 and time.time() < deadline:
        try:
            remove = (chips.first
                      .locator(SEL.reference_chip_root)
                      .locator(SEL.reference_chip_remove).first)
            remove.click(force=True, timeout=3_000)
    ...
    if remaining:
        _diagnose(page, "reference_clear_failed")
        raise RuntimeError(
            f"could not clear {remaining} pre-existing reference image(s) from "
            f"the form — refusing to generate with the wrong reference",
        )
...
# Public entry point
# ---------------------------------------------------------------------------
# OpenArt persists the whole Create-Image form (prompt + uploaded reference
# images) in this localStorage key, and Playwright's storage_state round-trips
# localStorage along with the auth cookies. Carrying it between runs meant every
# generation mounted the form with the PREVIOUS row's reference still attached:
# `_attach_reference_image` then appended the current row's image and Nano Banana
# Pro received two references, blending the wrong one. Question-image jobs, which
# attach nothing at all, silently inherited the leftover outright.
#
# Auth lives in cookies and the Clerk keys, so dropping this one key costs
# nothing and guarantees a clean form. Confirmed: clearing it AFTER the page
# loads does not work — the live zustand store rewrites it — so it has to be
# stripped from the state we hand to new_context().
FORM_STATE_KEY = "creation-store"


def _strip_form_state(state: dict) -> dict:
    """Return `state` without the persisted Create-Image form cache."""
    for origin in state.get("origins", []):
        entries = origin.get("localStorage")
        if not entries:
            continue
        origin["localStorage"] = [
            kv for kv in entries if kv.get("name") != FORM_STATE_KEY
        ]
    return state


def _load_storage_state() -> Optional[dict]:
    """Saved auth state with the stale form cache removed, or None on first run."""
    if not STATE_FILE.exists():
        return None
    try:
        state = json.loads(STATE_FILE.read_text())
    except (OSError, json.JSONDecodeError) as e:
        print(f"  ⚠ unreadable storage state ({e}) — starting signed out",
              file=sys.stderr)
        return None
    return _strip_form_state(state)


def _save_storage_state(context) -> None:
    """Persist auth state, minus the form cache, so the next run starts clean."""
    state = _strip_form_state(context.storage_state())
    ...
    with sync_playwright() as p, _browser(p, headless=headless) as ctx:
        ...
        def fill_and_submit(ws: Optional[str], precheck: bool) -> dict:
            ...
            if reference_image_path is not None:
                ...
                # No reference was asked for, so the form must not have one.
                # A question-image job attaches nothing, which used to let a
                # leftover chip ride along unnoticed into the generation.
                _clear_reference_images(page)
        ...
        for i, ws in enumerate(candidates):
            ...
            try:
                ...
                # workspace's user id).
                #
                # The reload alone does NOT empty the form: OpenArt rehydrates
                # it from its live store, so the first attempt's reference is
                # still attached and the retry's upload would stack on top of
                # it. Clear explicitly before refilling.
