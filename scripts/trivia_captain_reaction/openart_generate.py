#!/usr/bin/env python
"""Drive OpenArt to generate the avatar talking-head clip for one trivia-captain-reaction row.

Reads:
    projects/trivia-captain-reaction/<slug>/artifacts/script.json    # OpenArt prompt + duration + variant count
...
Routing:
    ...
    audio:       ON  (Seedance 2.0 native synced voice)
    output:      scripts/trivia_captain_reaction/library/clips/<slug>.mp4
                 (or <slug>_v1.mp4, <slug>_v2.mp4 when variants > 1)

NOTE: unlike trivia-captain, this pipeline uses NO reference image — it is a
pure character swap on ellie's reaction format (no tablet splash). The saved
`Captain Archibald` character carries identity; the per-row prompt never
describes his face.
...
Usage:
    ...
    python scripts/trivia_captain_reaction/openart_generate.py <slug> --headless
    python scripts/trivia_captain_reaction/openart_generate.py <slug> --force
    python scripts/trivia_captain_reaction/openart_generate.py <slug> --variants 1
"""
...
def _read_prompt_from_queue(slug: str) -> str:
    """Look up `slug` in the Posts_Reaction queue and return its OpenArt prompt (col J).
...
def _install_from_asset_id(args) -> int:
    ...
    saved = download_resource(
        ...
        audio_on=True,  # trivia-captain-reaction keeps Seedance native audio
    )
...
def main() -> int:
    ...
    # Source of truth for the prompt is the sheet (queue col J, written by the
    # script director). Older rows / dev machines may still have a local
    # script.json; we prefer that when present because it carries per-row
    # overrides for duration / model / character / resolution. When script.json
    # is absent, fall back to sheet + playbook defaults.
    ...
    if script_path.exists():
        ...
        if not prompt_from_sheet:
            sys.exit(
                ...
                f"script.json missing at {script_path} AND {queue_row.QUEUE_TAB}!J is empty. "
                f"Edit the prompt in {queue_row.QUEUE_TAB}!J before running."
            )
        openart_cfg = {"prompt": prompt_from_sheet}
        source = f"{queue_row.QUEUE_TAB}!J (sheet)"

    prompt = (openart_cfg.get("prompt") or "").strip()
    # Playbook defaults (styles/trivia-captain-reaction.yaml.asset_generation.openart).
    ...
    character = openart_cfg.get("character", "Captain Archibald")
    ...
    audio_on = bool(openart_cfg.get("audio_on", True))  # trivia-captain-reaction default
