"""Trivia captain reaction web UI (FastAPI sub-app, mounted by web/server.py at /trivia-captain-reaction).
...
<base href="/trivia-captain-reaction/"> would resolve incorrectly without the mount.
...
PKG_DIR = Path(__file__).resolve().parent.parent   # scripts/trivia_captain_reaction/
...
JobKind = Literal["select", "generate", "publish", "mark_published", "pull", "cover"]
...
async def _run_select(job: Job) -> None:
    ...
    py = sys.executable
    cmd = [py, "scripts/trivia_captain_reaction/select_row.py", "--day", str(day)]
    if slug_override:
...
async def _run_generate(job: Job) -> None:
    ...
    cmd = [
        py, "scripts/trivia_captain_reaction/openart_generate.py", job.slug,
        "--headless",
    ...
    rc = await _run_subprocess(
        job, [py, "scripts/trivia_captain_reaction/assemble.py", job.slug], REPO,
    )
    if rc != 0:
    ...
    rc = await _run_subprocess(
        ...
        [py, "scripts/common/transcribe.py", job.slug,
         "--root", "projects/trivia-captain-reaction"],
        REPO,
    )
    if rc != 0:
...
async def _run_publish(job: Job) -> None:
    ...
    py = sys.executable
    rc = await _run_subprocess(
        job, [py, "scripts/trivia_captain_reaction/publish.py", job.slug], REPO,
    )
    if rc != 0:
...
# FastAPI app
# ---------------------------------------------------------------------------
app = FastAPI(title="Trivia captain reaction runner")
...
async def api_save_prompt(payload: dict):
    ...
    prompt (cover_prompt → Queue!N, used verbatim by cover_gen)."""
    slug = (payload.get("slug") or "").strip()
...
async def api_cover_prompt_assemble(payload: dict):
    ...
    Playwright driver)."""
    from scripts.trivia_captain_reaction.cover_gen import build_prompt  # lazy
    prompt = build_prompt(
...
async def api_run(payload: dict):
    ...
    if kind not in ("select", "generate", "publish", "mark_published", "pull", "cover"):
