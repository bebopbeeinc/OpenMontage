#!/usr/bin/env python
"""Read one daily-trivia Day and write a brief artifact for the trivia-captain-reaction pipeline.
...
What it does:
    ...
    4. Write projects/trivia-captain-reaction/<slug>/artifacts/brief.json (schema-flexible; see metadata.trivia_captain_reaction).
    5. Upsert the Posts_Reaction row: Day, Slug, Status=Draft, Question, Correct Answer.

Humans curate which Days make good reaction-reel candidates; there is no
fact-fit heuristic (same as the sister trivia-reaction pipeline).

Usage:
    python scripts/trivia_captain_reaction/select_row.py --day 2
    python scripts/trivia_captain_reaction/select_row.py --day 2 --slug guinea-pigs-switzerland
    python scripts/trivia_captain_reaction/select_row.py --day 2 --dry-run
"""
...
from scripts.trivia_captain_reaction import daily_trivia, queue_row  # noqa: E402
...
def upsert_queue_row(
    ...
    """Find Posts_Reaction!Day=N or append a new row; return the 1-indexed sheet row."""
...
def main() -> int:
    ...
    brief = {
        ...
        "tone": "amused disbelief, intimate, single-take selfie reaction (70yo male world traveler)",
        "style": "trivia-captain-reaction",
        "target_platform": "instagram",
        "target_duration_seconds": 15.0,
        "metadata": {
            "sheet_revision": compute_revision_hash(trivia),
            "trivia_captain_reaction": {
                "day": args.day,
                "slug": slug,
    ...
    print(f"  queue: {queue_row.QUEUE_TAB}!row {qrow} → Status={queue_row.STATUS_DRAFT}")
    return 0
