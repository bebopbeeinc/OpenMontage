#!/usr/bin/env python
"""Assemble the trivia-captain-reaction bg video + meta for one row.
...
Inputs (preferred → fallback):
    projects/trivia-captain-reaction/<slug>/artifacts/brief.json   (optional; sheet_revision tag)
    projects/trivia-captain-reaction/<slug>/artifacts/script.json  (optional; beats)
    Posts_Reaction row matching <slug>                             (used when either file is missing)
    scripts/trivia_captain_reaction/library/clips/<slug>.mp4       (avatar clip from Seedance, audio inline)

Outputs:
    projects/trivia-captain-reaction/<slug>/assets/video/bg.mp4        (normalized 1080x1920 / h264 / AAC, audio preserved)
    projects/trivia-captain-reaction/<slug>/assets/meta.json           (consumed by Remotion TriviaWithBg)
    projects/trivia-captain-reaction/<slug>/artifacts/edit_decisions.json
...
showFactsOverlay=false + highlightColor=#C04FE0 (warm purple).

Usage:
    python scripts/trivia_captain_reaction/assemble.py <slug>
"""
...
def _find_queue_row(slug: str) -> dict | None:
    """Look up `slug` in the Posts_Reaction queue. Returns the row dict or None."""
    try:
...
def _queue_revision_hash(qrow: dict) -> str:
    ...
    D/F/G/H is reflected in edit_decisions.metadata.sheet_revision even when
    brief.json was never materialized locally."""
    payload = [
...
def main() -> int:
    ...
    ap.add_argument("slug", type=str, help="project slug (matches projects/trivia-captain-reaction/<slug>/)")
    args = ap.parse_args()
    ...
    # fall back to the Posts_Reaction queue (the workflow-state SoT) the same
    # way openart_generate.py does. Only fetch the queue row if we actually
    # need it, so the existing-files path keeps working offline.
    qrow: dict | None = None
    ...
    def _ensure_qrow() -> dict:
        ...
        if qrow is None:
            ...
            if qrow is None:
                sys.exit(
                    f"no brief.json/script.json on disk AND no {queue_row.QUEUE_TAB} row "
    ...
    if brief_path.exists():
        ...
        brief_source = f"{queue_row.QUEUE_TAB} (sheet)"
    ...
    if script_path.exists():
        ...
        script_source = f"{queue_row.QUEUE_TAB} (sheet)"
    ...
    if not ffprobe_has_audio(clip):
        sys.exit(
            f"⚠ {clip.name} has no audio stream. trivia-captain-reaction relies on Seedance "
    ...
    # The snake_case override fields (duration_s, highlight_color, ...) are
    # reaction-format extensions that trivia-short ignores.
    meta = {
        ...
        # --- reaction render overrides ---
        "duration_s": bg_dur,
        "highlight_color": "#C04FE0",   # warm purple pill (styles/trivia-captain-reaction.yaml)
