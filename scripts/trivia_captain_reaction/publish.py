...
def find_row(sheets, slug: str) -> tuple[int, dict]:
    ...
    for r in rows:
        if r.get("slug") == slug:
            return r["row"], r
    raise SystemExit(f"no {queue_row.QUEUE_TAB} row with slug={slug!r}; run select_row.py first")
