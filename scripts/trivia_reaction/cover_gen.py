...
Usage:
    python scripts/trivia_reaction/cover_gen.py <slug> \
        --headline "MOWING IS ILLEGAL?!" --highlight "ILLEGAL?!" \
        --emotion "laughing with her hand over her mouth" \
        --prop "a red 'no lawn-mowing' prohibition sign (a lawnmower crossed out with a red circle)" \
        [--frame-time 8.9] [--character ellie.travelcrush] [--variants 2] [--headless]
"""
...
def build_prompt(headline: str, highlight: str, emotion: str, prop: str,
    ...
    emo = emotion.strip() or "her exact expression and pose from the photo"
    return (
        "Vibrant vertical 9:16 YouTube-style thumbnail. Keep the woman EXACTLY "
        f"as she is — same face, same {emo}, same clothing, same identity, do "
        "not alter her features. Recompose so she sits in the lower-right of "
        "the frame, zoomed out slightly with clear headroom above her, leaving "
        ...
        f"overlap her face.{prop_clause} Warm cinematic lighting, subtle dark "
...
def main() -> int:
    ...
    ap.add_argument("--character", default="", help="OpenArt saved character (e.g. ellie.travelcrush) — locks her face when the scene is reimagined")
