#!/usr/bin/env python
"""Assemble the trivia-captain bg video + meta for one row.
...
Inputs (preferred → fallback):
    projects/trivia-captain/<slug>/artifacts/brief.json   (optional; sheet_revision tag)
    projects/trivia-captain/<slug>/artifacts/script.json  (optional; beats)
    TriviaCaptainQueue row matching <slug>                (used when either file is missing)
    scripts/trivia_captain/library/clips/<slug>.mp4       (avatar clip from Seedance, audio inline)

Outputs:
    projects/trivia-captain/<slug>/assets/video/bg.mp4        (normalized 1080x1920 / h264 / AAC, audio preserved)
    projects/trivia-captain/<slug>/assets/meta.json           (consumed by Remotion TriviaWithBg)
    projects/trivia-captain/<slug>/artifacts/edit_decisions.json
...
showFactsOverlay=false + highlightColor=#D63B2F.
...
def _find_queue_row(slug: str) -> dict | None:
    """Look up `slug` in TriviaCaptainQueue. Returns the row dict or None."""
    try:
...
def main() -> int:
    ...
    # fall back to TriviaCaptainQueue (the workflow-state SoT) the same way
    ...
    def _ensure_qrow() -> dict:
        ...
        if qrow is None:
            ...
            if qrow is None:
                sys.exit(
                    f"no brief.json/script.json on disk AND no TriviaCaptainQueue row "
    ...
    if brief_path.exists():
        ...
        brief_source = "TriviaCaptainQueue (sheet)"
    ...
    if script_path.exists():
        ...
        script_source = "TriviaCaptainQueue (sheet)"
    ...
    # are trivia-captain extensions that trivia-short ignores.
    meta = {
        ...
        # --- trivia-captain render overrides ---
        "duration_s": bg_dur,
        "highlight_color": "#D63B2F",
